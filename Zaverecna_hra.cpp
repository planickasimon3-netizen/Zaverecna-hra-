#include <iostream>
#include <string>
#include <ctime>
#include <cstdlib>
#include <cmath>

using namespace std;


int main(){

    srand(time(0));
    int vyber_utoku;
    int vyber_ability;
    int vyber_postavy;
    int vyber_ve_vesnici;
    int random_cislo_powerhit;
    int random_cislo_uhyb;

    struct postava{

    string povolani;
    string abilityName[3];
    string ability;
    int maxHp, hp;
    int maxMana, mana;
    int gold;
    int level;
    int xp;
    int attack;
    int abilityManaCost [3] = {1,2,3};
    int abilityDamage [3] = { 0,1,2};

    }hrac;


    struct Monster {
    string name;
    int hp;
    int attack;
    int powerhit;
    int goldDrop;
    int xpdrop;
    }goblin, goblin1, goblin2, goblin3, mbos, bbos;

    bbos.name = "Rostliny parazit";
    bbos.hp = 100;
    bbos.attack = 10;

    mbos.name = "Minibos";
    mbos.hp = 45;
    mbos.attack = 3;
    mbos.powerhit = 5;
    mbos.goldDrop = 5;
    mbos.xpdrop = 20;

    goblin.name = "Goblin";
    goblin.hp = 20;
    goblin.attack = 2;
    goblin.powerhit = 3;
    goblin.goldDrop = 1;
    goblin.xpdrop = 5;

    goblin1.name = "Goblin";
    goblin1.hp = 20;
    goblin1.attack = 2;
    goblin1.powerhit = 3;
    goblin1.goldDrop = 1;
    goblin1.xpdrop = 5;

    goblin2.name = "Goblin";
    goblin2.hp = 20;
    goblin2.attack = 2;
    goblin2.powerhit = 3;
    goblin2.goldDrop = 1;
    goblin2.xpdrop = 5;

    goblin3.name = "Goblin";
    goblin3.hp = 20;
    goblin3.attack = 2;
    goblin3.powerhit = 3;
    goblin3.goldDrop = 1;
    goblin3.xpdrop = 5;


    do {
        cout << "Zadej cislo sve postavy: 1. Utocnik, 2. Obrance, 3. Carodej, 4. Medic: " << endl;
        cin >> vyber_postavy;

        switch (vyber_postavy) {
            case 1:
                cout << "Zvolil jsi Utocnika!" << endl;
                hrac.povolani = "Utocnik";
                hrac.maxHp = 20; hrac.hp = 20;
                hrac.maxMana = 50; hrac.mana = 50;
                hrac.gold = 5; hrac.level = 1; hrac.xp = 0; hrac.attack = 10;
                hrac.ability = "1, Powerhit - cena(3 mana, 5 damage) 2, Basic attack - cena(0 mana, 2 damage) 3, Ultimate - cena(5 mana, 10 damage)";
                hrac.abilityName[0] = "Powerhit"; hrac.abilityName[1] = "Basic attack"; hrac.abilityName[2] = "Ultimate";
                hrac.abilityManaCost[0] = 3; hrac.abilityManaCost[1] = 0; hrac.abilityManaCost[2] = 5;
                hrac.abilityDamage[0] = 5; hrac.abilityDamage[1] = 2; hrac.abilityDamage[2] = 10;
                break;

            case 2:
                cout << "Zvolil jsi Obrance!" << endl << endl;
                hrac.povolani = "Obrance";
                hrac.maxHp = 25; hrac.hp = 25;
                hrac.maxMana = 40; hrac.mana = 40;
                hrac.gold = 5; hrac.level = 1; hrac.xp = 0;
                hrac.ability = "1, Shield - cena(2 mana, obrana na jedno kolo) 2, Defend attack - cena(0 mana, 2 damage) 3, Ultimate - cena(5 mana, 10 damage)";
                hrac.abilityName[0] = "Shield"; hrac.abilityName[1] = "Defend attack"; hrac.abilityName[2] = "Ultimate";
                hrac.abilityManaCost[0] = 2; hrac.abilityManaCost[1] = 0; hrac.abilityManaCost[2] = 5;
                hrac.abilityDamage[0] = 0; hrac.abilityDamage[1] = 2; hrac.abilityDamage[2] = 10;
                break;

            case 3:
                cout << "Zvolil jsi Carodeje!" << endl << endl;
                hrac.povolani = "Carodej";
                hrac.maxHp = 100; hrac.hp = 100;
                hrac.maxMana = 75; hrac.mana = 75;
                hrac.gold = 5; hrac.level = 1; hrac.xp = 0;
                hrac.ability = "1, Fireball - cena(5 mana, 10 damage) 2, Heal - cena(2 mana, 5 hp) 3, Ultimate - cena(10 mana, 20 damage)";

                hrac.abilityName[0] = "Fireball"; hrac.abilityName[1] = "Heal"; hrac.abilityName[2] = "Ultimate";
                hrac.abilityManaCost[0] = 5; hrac.abilityManaCost[1] = 2; hrac.abilityManaCost[2] = 10;

                hrac.abilityDamage[0] = 10; hrac.abilityDamage[1] = -5; hrac.abilityDamage[2] = 20;
                break;

            case 4:
                cout << "Zvolil jsi Medika!" << endl << endl;
                hrac.povolani = "Medic";
                hrac.maxHp = 18; hrac.hp = 18;
                hrac.maxMana = 50; hrac.mana = 50;
                hrac.gold = 5; hrac.level = 1; hrac.xp = 0;
                hrac.ability = "1, Heal - cena(3 mana, 5 hp) 2, Attack - cena(0 mana, 2 damage) 3, Ultimate - cena(7 mana, 15 damage)";

                hrac.abilityName[0] = "Heal"; hrac.abilityName[1] = "Attack"; hrac.abilityName[2] = "Ultimate";
                hrac.abilityManaCost[0] = 3; hrac.abilityManaCost[1] = 0; hrac.abilityManaCost[2] = 7;

                hrac.abilityDamage[0] = -5; hrac.abilityDamage[1] = 2; hrac.abilityDamage[2] = 15;
                break;

            default:
                cout << "Neplatne cislo, zkus to znovu." << endl;
        }

        if (vyber_postavy >= 1 && vyber_postavy <= 4) {
            cout << "Mate k dispozici tyto schopnosti: " << hrac.ability << endl;
            cout << "Mate " << hrac.hp << " zivotu a " << hrac.mana << " many." << endl;
            cout << "Mate " << hrac.gold << " goldu." << endl;
            cout << "Mate " << hrac.xp << " zkusenosti." << endl;
            cout << "Mate level " << hrac.level << "." << endl << endl;
        }

    }while(vyber_postavy < 1 || vyber_postavy > 4);

/*---1 VESNICE---*/

    do {
        cout << "Dosel jsi do vesnice, muzes si: 1: Doplnit zivoty(3 goldy)" << endl
             << "                                2: Zvetsit maximalni pocet zivotu o 2(5 goldu)" << endl
             << "                                3: Zvetsit maximalni manu o 5(5 goldu)" << endl
             << "                                4: Pokracovat do boje" << endl;
        cin >> vyber_ve_vesnici;

        if (vyber_ve_vesnici == 1) {
            hrac.hp = hrac.hp + 5;
            hrac.gold = hrac.gold - 3;
        } else if (vyber_ve_vesnici == 2) {
            hrac.maxHp = hrac.maxHp + 2;
            hrac.hp = hrac.hp + 2;
            hrac.gold = hrac.gold - 5;
        } else if (vyber_ve_vesnici == 3) {
            hrac.maxMana = hrac.maxMana + 5;
            hrac.mana = hrac.mana + 5;
            hrac.gold = hrac.gold - 5;
        } else if (vyber_ve_vesnici == 4) {
            cout << endl << "Pokracujes do boje se vsemi goldiky" << endl << endl;
        }else{cout << "Chyba zadali jste neplatne cislo" << endl;}


    } while(vyber_ve_vesnici < 1 || vyber_ve_vesnici > 5);

    /* --- První souboj s Goblinem --- */

    cout << "Narazil jsi na monstrum, musis s nim bojovat, jako abilitu chces pouzit: " << hrac.ability << endl;
    goblin.hp = 20;
    goblin1.hp = 20;
    goblin2.hp = 20;

    do {
        do {
            cout << endl << "Co mu provedeme ted: ";
            cin >> vyber_ability;
            random_cislo_powerhit = rand() % 100 + 1;
            random_cislo_uhyb = rand() % 100 + 1;

            if (vyber_postavy == 1) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Powerhit a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.hp = goblin.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Basic attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 2) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Shield a obranil jsi se na jedno kolo" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.attack = 0; goblin.powerhit = 0;
                        goblin1.attack = 0; goblin1.powerhit = 0;
                        goblin2.attack = 0; goblin2.powerhit = 0;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Defend attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 3) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Fireball a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.hp = goblin.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[1] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        hrac.hp = hrac.hp - hrac.abilityDamage[1]; // Minus a minus dává plus (léčení)
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 4) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[0] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        hrac.hp = hrac.hp - hrac.abilityDamage[0];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }
            }

            cout << "Zbylo ti: " << hrac.hp << "hp" << endl;
            cout << "Mas " << hrac.mana << " many" << endl;

        } while(vyber_ability < 1 || vyber_ability > 3);
    } while(goblin.hp > 0 && hrac.hp > 0);

    if(hrac.hp < 1){
        cout << "UMREL JSI";
        return 0;
    }

    hrac.mana = hrac.mana + 5;
    hrac.xp = hrac.xp + goblin.xpdrop;
    hrac.gold = hrac.gold + goblin.goldDrop;


    /*   ---Souboj s druhým goblinem---  */

    cout << endl << "Narazil jsi na monstrum, musis s nim bojovat, jako abilitu chces pouzit: " << hrac.ability << endl;
    goblin.hp = 20;
    goblin1.hp = 20;
    goblin2.hp = 20;

    do {
        do {
            cout << endl << "Co mu provedeme ted: ";
            cin >> vyber_ability;
            random_cislo_powerhit = rand() % 100 + 1;
            random_cislo_uhyb = rand() % 100 + 1;

            if (vyber_postavy == 1) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Powerhit a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.hp = goblin.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Basic attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 2) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Shield a obranil jsi se na jedno kolo" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.attack = 0; goblin.powerhit = 0;
                        goblin1.attack = 0; goblin1.powerhit = 0;
                        goblin2.attack = 0; goblin2.powerhit = 0;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Defend attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 3) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Fireball a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.hp = goblin.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[1] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        hrac.hp = hrac.hp - hrac.abilityDamage[1];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 4) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[0] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        hrac.hp = hrac.hp - hrac.abilityDamage[0];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }
            }

            cout << "Zbylo ti: " << hrac.hp << "hp" << endl;
            cout << "Mas " << hrac.mana << " many" << endl;

        } while(vyber_ability < 1 || vyber_ability > 3);
    } while(goblin.hp > 0 && hrac.hp > 0);

    if(hrac.hp < 1){
        cout << "UMREL JSI";
        return 0;
    }

    hrac.mana = hrac.mana + 10;
    hrac.xp = hrac.xp + goblin.xpdrop;
    hrac.gold = hrac.gold + goblin.goldDrop;


    /*   ---Třetí souboj se třemi Gobliny---  */

    cout << endl << "Narazil jsi na 3 monstra, musis s nim bojovat, jako abilitu chces pouzit: " << hrac.ability << endl;
    goblin.hp = 20;
    goblin1.hp = 20;
    goblin2.hp = 20;

do{

    do{
        cout << "Na jakeho z nich budes utocit: " << endl;
        cin >> vyber_utoku;


    if(vyber_utoku == 1){


    do {
        do {
            while(goblin.hp > 1){

            cout << endl << "Co mu provedeme ted: ";
            cin >> vyber_ability;
            random_cislo_powerhit = rand() % 100 + 1;
            random_cislo_uhyb = rand() % 100 + 1;

            if (vyber_postavy == 1) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Powerhit a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.hp = goblin.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Basic attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 2) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Shield a obranil jsi se na jedno kolo" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.attack = 0; goblin.powerhit = 0;
                        goblin1.attack = 0; goblin1.powerhit = 0;
                        goblin2.attack = 0; goblin2.powerhit = 0;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Defend attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 3) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Fireball a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.hp = goblin.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[1] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        hrac.hp = hrac.hp - hrac.abilityDamage[1];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 4) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[0] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        hrac.hp = hrac.hp - hrac.abilityDamage[0];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }
            }

            cout << "Zbylo ti: " << hrac.hp << "hp" << endl;
            cout << "Mas " << hrac.mana << " many" << endl;
        }
        } while(vyber_ability < 1 || vyber_ability > 3);
    } while(goblin.hp > 0 && hrac.hp > 0);

    if(hrac.hp < 1){
        cout << "UMREL JSI";
        return 0;
    }

    hrac.mana = hrac.mana + 10;
    hrac.xp = hrac.xp + goblin.xpdrop;
    hrac.gold = hrac.gold + goblin.goldDrop;



    }else if(vyber_utoku == 2){



    do {
        do {
            while(goblin1.hp > 1){
            cout << endl << "Co mu provedeme ted: ";
            cin >> vyber_ability;
            random_cislo_powerhit = rand() % 100 + 1;
            random_cislo_uhyb = rand() % 100 + 1;

            if (vyber_postavy == 1) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Powerhit a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Basic attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 2) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Shield a obranil jsi se na jedno kolo" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.attack = 0; goblin.powerhit = 0;
                        goblin1.attack = 0; goblin1.powerhit = 0;
                        goblin2.attack = 0; goblin2.powerhit = 0;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Defend attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 3) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Fireball a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[1] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        hrac.hp = hrac.hp - hrac.abilityDamage[1];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 4) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[0] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        hrac.hp = hrac.hp - hrac.abilityDamage[0];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }
            }

            cout << "Zbylo ti: " << hrac.hp << "hp" << endl;
            cout << "Mas " << hrac.mana << " many" << endl;
                }
        } while(vyber_ability < 1 || vyber_ability > 3);
    } while(goblin1.hp > 0 && hrac.hp > 0);

    if(hrac.hp < 1){
        cout << "UMREL JSI";
        return 0;
    }

    hrac.mana = hrac.mana + 10;
    hrac.xp = hrac.xp + goblin1.xpdrop;
    hrac.gold = hrac.gold + goblin1.goldDrop;


    }else if(vyber_utoku == 3){


    do {
        do {
            while(goblin2.hp > 1){


            cout << endl << "Co mu provedeme ted: ";
            cin >> vyber_ability;
            random_cislo_powerhit = rand() % 100 + 1;
            random_cislo_uhyb = rand() % 100 + 1;

            if (vyber_postavy == 1) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Powerhit a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Basic attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 2) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Shield a obranil jsi se na jedno kolo" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.attack = 0; goblin.powerhit = 0;
                        goblin1.attack = 0; goblin1.powerhit = 0;
                        goblin2.attack = 0; goblin2.powerhit = 0;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Defend attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 3) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Fireball a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[1] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        hrac.hp = hrac.hp - hrac.abilityDamage[1];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 4) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[0] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        hrac.hp = hrac.hp - hrac.abilityDamage[0];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }
            }

            cout << "Zbylo ti: " << hrac.hp << "hp" << endl;
            cout << "Mas " << hrac.mana << " many" << endl;


    hrac.mana = hrac.mana + 10;
    hrac.xp = hrac.xp + goblin2.xpdrop;
    hrac.gold = hrac.gold + goblin2.goldDrop;
        }
        }while(vyber_ability < 1 || vyber_ability > 3);
    }while(goblin2.hp > 0 && hrac.hp > 0);

    if(hrac.hp < 1){
        cout << "UMREL JSI";
        return 0;
    }


    }

}while(goblin.hp > 1 || goblin1.hp > 1 || goblin2.hp > 1 );
}while(vyber_utoku < 1 || vyber_utoku > 3);

/*---LEVEL UPGRADE 1*/

cout << endl << "Dosahnul jsi druheho levelu, vsechny ability jsou vylepseny o 20 procent" << endl;
hrac.abilityDamage[0] = hrac.abilityDamage[0] + (hrac.abilityDamage[0]/10)*2;
hrac.abilityDamage[1] = hrac.abilityDamage[1] + (hrac.abilityDamage[1]/10)*2;
hrac.abilityDamage[2] = hrac.abilityDamage[2] + (hrac.abilityDamage[2]/10)*2;

/*---2 VESNICE---*/

do {
        cout << endl << "Mas k dispozici " << hrac.gold << "goldu";
        cout << endl << "Dosel jsi do vesnice, muzes si: 1: Doplnit zivoty(3 goldy)" << endl
             << "                                2: Zvetsit maximalni pocet zivotu o 2(5 goldu)" << endl
             << "                                3: Zvetsit maximalni manu o 5(5 goldu)" << endl
             << "                                4: Pokracovat do boje" << endl;
        cin >> vyber_ve_vesnici;

        if (vyber_ve_vesnici == 1) {
            hrac.hp = hrac.hp + 5;
            hrac.gold = hrac.gold - 3;
        } else if (vyber_ve_vesnici == 2) {
            hrac.maxHp = hrac.maxHp + 2;
            hrac.hp = hrac.hp + 2;
            hrac.gold = hrac.gold - 5;
        } else if (vyber_ve_vesnici == 3) {
            hrac.maxMana = hrac.maxMana + 5;
            hrac.mana = hrac.mana + 5;
            hrac.gold = hrac.gold - 5;
        } else if (vyber_ve_vesnici == 4) {
            cout << endl << "Pokracujes do boje se vsemi goldiky" << endl << endl;
        }else{cout << "Chyba zadali jste neplatne cislo" << endl;}


}while(vyber_ve_vesnici < 1 || vyber_ve_vesnici > 5);

   /*--- 4 souboj MINIBOS--- */

cout << endl << "POTKAL JSI MINIBOSE" << endl;

do {
        do {

            cout << endl << "Co mu provedes ted: ";
            cin >> vyber_ability;
            random_cislo_powerhit = rand() % 100 + 1;
            random_cislo_uhyb = rand() % 100 + 1;

            if (vyber_postavy == 1) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Powerhit a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        mbos.hp = mbos.hp - hrac.abilityDamage[0];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Basic attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        mbos.hp = mbos.hp - hrac.abilityDamage[1];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        mbos.hp = mbos.hp - hrac.abilityDamage[2];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, Minibos se te ani nedotknul" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Minibos ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Minibos ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 2) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Shield a obranil jsi se na jedno kolo" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.attack = 0; goblin.powerhit = 0;
                        goblin1.attack = 0; goblin1.powerhit = 0;
                        mbos.attack = 0; mbos.powerhit = 0;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Defend attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        mbos.hp = mbos.hp - hrac.abilityDamage[1];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        mbos.hp = mbos.hp - hrac.abilityDamage[2];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, Minibos se te ani nedotknul" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Minibos ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Minibos ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 3) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Fireball a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        mbos.hp = mbos.hp - hrac.abilityDamage[0];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[1] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        hrac.hp = hrac.hp - hrac.abilityDamage[1];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        mbos.hp = mbos.hp - hrac.abilityDamage[2];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, Minibos se te ani nedotknul" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Minibos ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Minibos ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 4) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[0] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        hrac.hp = hrac.hp - hrac.abilityDamage[0];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        mbos.hp = mbos.hp - hrac.abilityDamage[1];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        mbos.hp = mbos.hp - hrac.abilityDamage[2];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, Minibos se te ani nedotknul" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Minibos ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Minibos ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }
            }

            cout << "Zbylo ti: " << hrac.hp << "hp" << endl;
            cout << "Mas " << hrac.mana << " many" << endl;



        }while(vyber_ability < 1 || vyber_ability > 3);
    }while(mbos.hp > 0 && hrac.hp > 1);
    if(hrac.hp < 1){
        cout << "UMREL JSI";
        return 0;
    }

    hrac.mana = hrac.mana + 20;
    hrac.xp = hrac.xp + mbos.xpdrop;
    hrac.gold = hrac.gold + mbos.goldDrop;

/*---5 SOUBOJ S JEDNÍM MONSTREM--*/

cout << endl << "Narazil jsi na monstrum, musis s nim bojovat, jako abilitu chces pouzit: " << hrac.ability << endl;
    goblin.hp = 20;
    goblin1.hp = 20;
    goblin2.hp = 20;

    do {
        do {
            cout << endl << "Co mu provedeme ted: ";
            cin >> vyber_ability;
            random_cislo_powerhit = rand() % 100 + 1;
            random_cislo_uhyb = rand() % 100 + 1;

            if (vyber_postavy == 1) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Powerhit a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.hp = goblin.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Basic attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 2) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Shield a obranil jsi se na jedno kolo" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.attack = 0; goblin.powerhit = 0;
                        goblin1.attack = 0; goblin1.powerhit = 0;
                        goblin2.attack = 0; goblin2.powerhit = 0;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Defend attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 3) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Fireball a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.hp = goblin.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[1] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        hrac.hp = hrac.hp - hrac.abilityDamage[1]; // Minus a minus dává plus (léčení)
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 4) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[0] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        hrac.hp = hrac.hp - hrac.abilityDamage[0];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }
            }

            cout << "Zbylo ti: " << hrac.hp << "hp" << endl;
            cout << "Mas " << hrac.mana << " many" << endl;

        } while(vyber_ability < 1 || vyber_ability > 3);
    } while(goblin.hp > 0 && hrac.hp > 0);

    if(hrac.hp < 1){
        cout << "UMREL JSI";
        return 0;
    }

    hrac.mana = hrac.mana + 10;
    hrac.xp = hrac.xp + goblin.xpdrop;
    hrac.gold = hrac.gold + goblin.goldDrop;

/*---LEVEL UPGRADE 2*/

cout << endl << "Dosahnul jsi tretiho levelu, vsechny ability jsou vylepseny o 20 procent" << endl;
hrac.abilityDamage[0] = hrac.abilityDamage[0] + (hrac.abilityDamage[0]/10)*2;
hrac.abilityDamage[1] = hrac.abilityDamage[1] + (hrac.abilityDamage[1]/10)*2;
hrac.abilityDamage[2] = hrac.abilityDamage[2] + (hrac.abilityDamage[2]/10)*2;

/*---3 VESNICE---*/

    do{
        cout << endl << "Mas k dispozici " << hrac.gold << "goldu";
        cout << endl << "Dosel jsi do vesnice, muzes si: 1: Doplnit zivoty(3 goldy)" << endl
             << "                                2: Zvetsit maximalni pocet zivotu o 2(5 goldu)" << endl
             << "                                3: Zvetsit maximalni manu o 5(5 goldu)" << endl
             << "                                4: Pokracovat do boje" << endl;
        cin >> vyber_ve_vesnici;

        if (vyber_ve_vesnici == 1) {
            hrac.hp = hrac.hp + 5;
            hrac.gold = hrac.gold - 3;
        } else if (vyber_ve_vesnici == 2) {
            hrac.maxHp = hrac.maxHp + 2;
            hrac.hp = hrac.hp + 2;
            hrac.gold = hrac.gold - 5;
        } else if (vyber_ve_vesnici == 3) {
            hrac.maxMana = hrac.maxMana + 5;
            hrac.mana = hrac.mana + 5;
            hrac.gold = hrac.gold - 5;
        } else if (vyber_ve_vesnici == 4) {
            cout << endl << "Pokracujes do boje se vsemi goldiky" << endl << endl;
        }else{cout << "Chyba zadali jste neplatne cislo" << endl;}


}while(vyber_ve_vesnici < 1 || vyber_ve_vesnici > 5);

/*---6 SOUBOJ SE TŘEMI MONSTRY---*/

    cout << endl << "Narazil jsi na 3 monstra, musis s nim bojovat, jako abilitu chces pouzit: " << hrac.ability << endl;
    goblin.hp = 20;
    goblin1.hp = 20;
    goblin2.hp = 20;

do{

    do{
        cout << "Na jakeho z nich budes utocit: " << endl;
        cin >> vyber_utoku;


    if(vyber_utoku == 1){


    do {
        do {
            while(goblin.hp > 1){

            cout << endl << "Co mu provedeme ted: ";
            cin >> vyber_ability;
            random_cislo_powerhit = rand() % 100 + 1;
            random_cislo_uhyb = rand() % 100 + 1;

            if (vyber_postavy == 1) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Powerhit a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.hp = goblin.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Basic attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 2) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Shield a obranil jsi se na jedno kolo" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.attack = 0; goblin.powerhit = 0;
                        goblin1.attack = 0; goblin1.powerhit = 0;
                        goblin2.attack = 0; goblin2.powerhit = 0;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Defend attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 3) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Fireball a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.hp = goblin.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[1] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        hrac.hp = hrac.hp - hrac.abilityDamage[1]; // Minus a minus dává plus (léčení)
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 4) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[0] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        hrac.hp = hrac.hp - hrac.abilityDamage[0];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }
            }

            cout << "Zbylo ti: " << hrac.hp << "hp" << endl;
            cout << "Mas " << hrac.mana << " many" << endl;
        }
        } while(vyber_ability < 1 || vyber_ability > 3);
    } while(goblin.hp > 0 && hrac.hp > 0);

    if(hrac.hp < 1){
        cout << "UMREL JSI";
        return 0;
    }

    hrac.mana = hrac.mana + 10;
    hrac.xp = hrac.xp + goblin.xpdrop;
    hrac.gold = hrac.gold + goblin.goldDrop;



    }else if(vyber_utoku == 2){



    do {
        do {
            while(goblin1.hp > 1){
            cout << endl << "Co mu provedeme ted: ";
            cin >> vyber_ability;
            random_cislo_powerhit = rand() % 100 + 1;
            random_cislo_uhyb = rand() % 100 + 1;

            if (vyber_postavy == 1) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Powerhit a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Basic attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 2) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Shield a obranil jsi se na jedno kolo" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.attack = 0; goblin.powerhit = 0;
                        goblin1.attack = 0; goblin1.powerhit = 0;
                        goblin2.attack = 0; goblin2.powerhit = 0;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Defend attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 3) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Fireball a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[1] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        hrac.hp = hrac.hp - hrac.abilityDamage[1];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 4) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[0] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        hrac.hp = hrac.hp - hrac.abilityDamage[0];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }
            }

            cout << "Zbylo ti: " << hrac.hp << "hp" << endl;
            cout << "Mas " << hrac.mana << " many" << endl;
                }
        } while(vyber_ability < 1 || vyber_ability > 3);
    } while(goblin1.hp > 0 && hrac.hp > 0);

    if(hrac.hp < 1){
        cout << "UMREL JSI";
        return 0;
    }

    hrac.mana = hrac.mana + 10;
    hrac.xp = hrac.xp + goblin1.xpdrop;
    hrac.gold = hrac.gold + goblin1.goldDrop;


    }else if(vyber_utoku == 3){


    do {
        do {
            while(goblin2.hp > 1){


            cout << endl << "Co mu provedeme ted: ";
            cin >> vyber_ability;
            random_cislo_powerhit = rand() % 100 + 1;
            random_cislo_uhyb = rand() % 100 + 1;

            if (vyber_postavy == 1) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Powerhit a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Basic attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 2) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Shield a obranil jsi se na jedno kolo" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.attack = 0; goblin.powerhit = 0;
                        goblin1.attack = 0; goblin1.powerhit = 0;
                        goblin2.attack = 0; goblin2.powerhit = 0;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Defend attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 3) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Fireball a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[1] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        hrac.hp = hrac.hp - hrac.abilityDamage[1];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 4) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[0] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        hrac.hp = hrac.hp - hrac.abilityDamage[0];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin2.hp = goblin2.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin2.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }
            }

            cout << "Zbylo ti: " << hrac.hp << "hp" << endl;
            cout << "Mas " << hrac.mana << " many" << endl;


    hrac.mana = hrac.mana + 10;
    hrac.xp = hrac.xp + goblin2.xpdrop;
    hrac.gold = hrac.gold + goblin2.goldDrop;
        }
        }while(vyber_ability < 1 || vyber_ability > 3);
    }while(goblin2.hp > 0 && hrac.hp > 0);

    if(hrac.hp < 1){
        cout << "UMREL JSI";
        return 0;
    }


    }

}while(goblin.hp > 1 || goblin1.hp > 1 || goblin2.hp > 1 );
}while(vyber_utoku < 1 || vyber_utoku > 3);

/*---7 SOUBOJ SE DVĚMI MONSTRY---*/

 cout << endl << "Narazil jsi na 2 monstra, musis s nim bojovat, jako abilitu chces pouzit: " << hrac.ability << endl;
    goblin.hp = 20;
    goblin1.hp = 20;

do{

    do{
        cout << "Na jakeho z nich budes utocit: " << endl;
        cin >> vyber_utoku;


    if(vyber_utoku == 1){


    do {
        do {
            while(goblin.hp > 1){

            cout << endl << "Co mu provedeme ted: ";
            cin >> vyber_ability;
            random_cislo_powerhit = rand() % 100 + 1;
            random_cislo_uhyb = rand() % 100 + 1;

            if (vyber_postavy == 1) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Powerhit a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.hp = goblin.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Basic attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 2) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Shield a obranil jsi se na jedno kolo" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.attack = 0; goblin.powerhit = 0;
                        goblin1.attack = 0; goblin1.powerhit = 0;
                        goblin2.attack = 0; goblin2.powerhit = 0;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Defend attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 3) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Fireball a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.hp = goblin.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[1] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        hrac.hp = hrac.hp - hrac.abilityDamage[1]; // Minus a minus dává plus (léčení)
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 4) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[0] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        hrac.hp = hrac.hp - hrac.abilityDamage[0];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }
            }

            cout << "Zbylo ti: " << hrac.hp << "hp" << endl;
            cout << "Mas " << hrac.mana << " many" << endl;
        }
        } while(vyber_ability < 1 || vyber_ability > 3);
    } while(goblin.hp > 0 && hrac.hp > 0);

    if(hrac.hp < 1){
        cout << "UMREL JSI";
        return 0;
    }

    hrac.mana = hrac.mana + 10;
    hrac.xp = hrac.xp + goblin.xpdrop;
    hrac.gold = hrac.gold + goblin.goldDrop;



    }else if(vyber_utoku == 2){



    do {
        do {
            while(goblin1.hp > 1){
            cout << endl << "Co mu provedeme ted: ";
            cin >> vyber_ability;
            random_cislo_powerhit = rand() % 100 + 1;
            random_cislo_uhyb = rand() % 100 + 1;

            if (vyber_postavy == 1) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Powerhit a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Basic attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 2) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Shield a obranil jsi se na jedno kolo" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.attack = 0; goblin.powerhit = 0;
                        goblin1.attack = 0; goblin1.powerhit = 0;
                        goblin2.attack = 0; goblin2.powerhit = 0;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Defend attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 3) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Fireball a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[1] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        hrac.hp = hrac.hp - hrac.abilityDamage[1];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 4) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[0] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        hrac.hp = hrac.hp - hrac.abilityDamage[0];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin1.hp = goblin1.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin1.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }
            }

            cout << "Zbylo ti: " << hrac.hp << "hp" << endl;
            cout << "Mas " << hrac.mana << " many" << endl;
                }
        } while(vyber_ability < 1 || vyber_ability > 3);
    } while(goblin1.hp > 0 && hrac.hp > 0);

    if(hrac.hp < 1){
        cout << "UMREL JSI";
        return 0;
    }

    hrac.mana = hrac.mana + 10;
    hrac.xp = hrac.xp + goblin1.xpdrop;
    hrac.gold = hrac.gold + goblin1.goldDrop;

    }

}while(goblin.hp > 1 || goblin1.hp > 1);
}while(vyber_utoku < 1 || vyber_utoku > 2);

/*---LEVEL UPGRADE 3*/

cout << endl << "Dosahnul jsi ctvrteho levelu, vsechny ability jsou vylepseny o 20 procent" << endl;
hrac.abilityDamage[0] = hrac.abilityDamage[0] + (hrac.abilityDamage[0]/10)*2;
hrac.abilityDamage[1] = hrac.abilityDamage[1] + (hrac.abilityDamage[1]/10)*2;
hrac.abilityDamage[2] = hrac.abilityDamage[2] + (hrac.abilityDamage[2]/10)*2;

/*---4 VESNICE---*/

    do {
        cout << "Dosel jsi do vesnice, muzes si: 1: Doplnit zivoty(3 goldy)" << endl
             << "                                2: Zvetsit maximalni pocet zivotu o 2(5 goldu)" << endl
             << "                                3: Zvetsit maximalni manu o 5(5 goldu)" << endl
             << "                                4: Pokracovat do boje" << endl;
        cin >> vyber_ve_vesnici;

        if (vyber_ve_vesnici == 1) {
            hrac.hp = hrac.hp + 5;
            hrac.gold = hrac.gold - 3;
        } else if (vyber_ve_vesnici == 2) {
            hrac.maxHp = hrac.maxHp + 2;
            hrac.hp = hrac.hp + 2;
            hrac.gold = hrac.gold - 5;
        } else if (vyber_ve_vesnici == 3) {
            hrac.maxMana = hrac.maxMana + 5;
            hrac.mana = hrac.mana + 5;
            hrac.gold = hrac.gold - 5;
        } else if (vyber_ve_vesnici == 4) {
            cout << endl << "Pokracujes do boje se vsemi goldiky" << endl << endl;
        }else{cout << "Chyba zadali jste neplatne cislo" << endl;}


    } while(vyber_ve_vesnici < 1 || vyber_ve_vesnici > 5);


/*---8 SOUBOJ S MINIBOSEM---*/

cout << endl << "POTKAL JSI MINIBOSE" << endl;

do {
        do {

            cout << endl << "Co mu provedes ted: ";
            cin >> vyber_ability;
            random_cislo_powerhit = rand() % 100 + 1;
            random_cislo_uhyb = rand() % 100 + 1;

            if (vyber_postavy == 1) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Powerhit a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        mbos.hp = mbos.hp - hrac.abilityDamage[0];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Basic attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        mbos.hp = mbos.hp - hrac.abilityDamage[1];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        mbos.hp = mbos.hp - hrac.abilityDamage[2];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, Minibos se te ani nedotknul" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Minibos ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Minibos ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 2) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Shield a obranil jsi se na jedno kolo" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.attack = 0; goblin.powerhit = 0;
                        goblin1.attack = 0; goblin1.powerhit = 0;
                        mbos.attack = 0; mbos.powerhit = 0;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Defend attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        mbos.hp = mbos.hp - hrac.abilityDamage[1];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        mbos.hp = mbos.hp - hrac.abilityDamage[2];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, Minibos se te ani nedotknul" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Minibos ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Minibos ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 3) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Fireball a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        mbos.hp = mbos.hp - hrac.abilityDamage[0];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[1] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        hrac.hp = hrac.hp - hrac.abilityDamage[1];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        mbos.hp = mbos.hp - hrac.abilityDamage[2];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, Minibos se te ani nedotknul" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Minibos ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Minibos ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 4) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[0] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        hrac.hp = hrac.hp - hrac.abilityDamage[0];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        mbos.hp = mbos.hp - hrac.abilityDamage[1];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        mbos.hp = mbos.hp - hrac.abilityDamage[2];
                        cout << "Minibosovi zbylo " << mbos.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, Minibos se te ani nedotknul" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Minibos ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Minibos ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }
            }

            cout << "Zbylo ti: " << hrac.hp << "hp" << endl;
            cout << "Mas " << hrac.mana << " many" << endl;



        }while(vyber_ability < 1 || vyber_ability > 3);
    }while(mbos.hp > 0 && hrac.hp > 1);
    if(hrac.hp < 1){
        cout << "UMREL JSI";
        return 0;
    }

    hrac.mana = hrac.mana + 20;
    hrac.xp = hrac.xp + mbos.xpdrop;
    hrac.gold = hrac.gold + mbos.goldDrop;

    /*---9 SOUBOJ S JEDNÍM MONSTREM--*/

cout << endl << "Narazil jsi na monstrum, musis s nim bojovat, jako abilitu chces pouzit: " << hrac.ability << endl;
    goblin.hp = 20;
    goblin1.hp = 20;
    goblin2.hp = 20;

    do {
        do {
            cout << endl << "Co mu provedeme ted: ";
            cin >> vyber_ability;
            random_cislo_powerhit = rand() % 100 + 1;
            random_cislo_uhyb = rand() % 100 + 1;

            if (vyber_postavy == 1) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Powerhit a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.hp = goblin.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Basic attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 2) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Shield a obranil jsi se na jedno kolo" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.attack = 0; goblin.powerhit = 0;
                        goblin1.attack = 0; goblin1.powerhit = 0;
                        goblin2.attack = 0; goblin2.powerhit = 0;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Defend attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 3) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Fireball a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        goblin.hp = goblin.hp - hrac.abilityDamage[0];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[1] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        hrac.hp = hrac.hp - hrac.abilityDamage[1]; // Minus a minus dává plus (léčení)
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }

            } else if (vyber_postavy == 4) {
                if (vyber_ability == 1) {
                    if (hrac.mana >= hrac.abilityManaCost[0]) {
                        cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[0] << " hp" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[0];
                        hrac.hp = hrac.hp - hrac.abilityDamage[0];
                        cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 2) {
                    if (hrac.mana >= hrac.abilityManaCost[1]) {
                        cout << "Pouzil jsi Attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[1];
                        goblin.hp = goblin.hp - hrac.abilityDamage[1];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                } else if (vyber_ability == 3) {
                    if (hrac.mana >= hrac.abilityManaCost[2]) {
                        cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
                        hrac.mana = hrac.mana - hrac.abilityManaCost[2];
                        goblin.hp = goblin.hp - hrac.abilityDamage[2];
                        cout << "Goblinovi zbylo " << goblin.hp << " hp" << endl;
                    } else { cout << "Nemate dost many, zvolte jinou abilitu" << endl; }
                }

                if (random_cislo_uhyb <= 30) {
                    cout << "Uhybl jsi, goblin te netrefil" << endl;
                }else if(random_cislo_powerhit >= 31 || random_cislo_powerhit <= 80){
                    cout << "Goblin ti dal powerhit" << endl;
                    hrac.hp = hrac.hp - goblin.powerhit;
                }else if (random_cislo_powerhit >= 81){
                    cout << "Goblin ti dal normalni utok" << endl;
                    hrac.hp = hrac.hp - goblin.attack;
                }
            }

            cout << "Zbylo ti: " << hrac.hp << "hp" << endl;
            cout << "Mas " << hrac.mana << " many" << endl;

        } while(vyber_ability < 1 || vyber_ability > 3);
    } while(goblin.hp > 0 && hrac.hp > 0);

    if(hrac.hp < 1){
        cout << "UMREL JSI";
        return 0;
    }

    hrac.mana = hrac.mana + 10;
    hrac.xp = hrac.xp + goblin.xpdrop;
    hrac.gold = hrac.gold + goblin.goldDrop;

    /*---LEVEL UPGRADE 4*/

cout << endl << "Dosahnul jsi pateho levelu, vsechny ability jsou vylepseny o 20 procent" << endl;
hrac.abilityDamage[0] = hrac.abilityDamage[0] + (hrac.abilityDamage[0]/10)*2;
hrac.abilityDamage[1] = hrac.abilityDamage[1] + (hrac.abilityDamage[1]/10)*2;
hrac.abilityDamage[2] = hrac.abilityDamage[2] + (hrac.abilityDamage[2]/10)*2;

/*---10 SOUBOJ SE ČTYŘMI MONSTRY---*/

    cout << endl << "Narazil jsi na 4 monstra, musis s nim bojovat, jako abilitu chces pouzit: " << hrac.ability << endl;
    goblin.hp = 20;
    goblin1.hp = 20;
    goblin2.hp = 20;
    goblin3.hp = 20;

    while ((goblin.hp > 0 || goblin1.hp > 0 || goblin2.hp > 0 || goblin3.hp > 0) && hrac.hp > 0) {

        cout << "Na jakeho z nich budes utocit: " << endl;
        cin >> vyber_utoku;

        Monster* cil = nullptr;

        if (vyber_utoku == 1) {
            cil = &goblin;
        } else if (vyber_utoku == 2) {
            cil = &goblin1;
        } else if (vyber_utoku == 3) {
            cil = &goblin2;
        } else if (vyber_utoku == 4) {
            cil = &goblin3;
        } else {
            cout << "Neplatna volba, zadej cislo 1 az 4." << endl;
            continue;
        }

        if (cil->hp <= 0) {
            cout << "Tento goblin uz je mrtvy, vyber jine monstrum." << endl;
            continue;
        }

        while (cil->hp > 0 && hrac.hp > 0) {
            cout << endl << "Co mu provedeme ted: ";
            cin >> vyber_ability;

            if (vyber_ability < 1 || vyber_ability > 3) {
                cout << "Neplatna abilita, zadej cislo 1 az 3." << endl;
                continue;
            }

            if (hrac.mana < hrac.abilityManaCost[vyber_ability - 1]) {
                cout << "Nemate dost many, zvolte jinou abilitu" << endl;
                continue;
            }

            hrac.mana = hrac.mana - hrac.abilityManaCost[vyber_ability - 1];

            if (hrac.abilityDamage[vyber_ability - 1] < 0) {
                cout << "Pouzil jsi " << hrac.abilityName[vyber_ability - 1]
                     << " a vylecil jsi se o " << -hrac.abilityDamage[vyber_ability - 1] << " hp" << endl;
                hrac.hp = hrac.hp - hrac.abilityDamage[vyber_ability - 1];
                if (hrac.hp > hrac.maxHp) hrac.hp = hrac.maxHp;
                cout << "Tobe zbylo " << hrac.hp << " hp" << endl;
            } else {
                cout << "Pouzil jsi " << hrac.abilityName[vyber_ability - 1]
                     << " a udelil jsi " << hrac.abilityDamage[vyber_ability - 1] << " damage" << endl;
                cil->hp = cil->hp - hrac.abilityDamage[vyber_ability - 1];
                cout << "Goblinovi zbylo " << cil->hp << " hp" << endl;
            }

            if (cil->hp <= 0) {
                cout << "Goblin byl porazen!" << endl;
                hrac.mana = hrac.mana + 10;
                hrac.xp = hrac.xp + cil->xpdrop;
                hrac.gold = hrac.gold + cil->goldDrop;
                break;
            }

            random_cislo_powerhit = rand() % 100 + 1;
            random_cislo_uhyb = rand() % 100 + 1;

            if (random_cislo_uhyb <= 30) {
                cout << "Uhybl jsi, goblin te netrefil" << endl;
            } else if (random_cislo_powerhit >= 31 && random_cislo_powerhit <= 80) {
                cout << "Goblin ti dal powerhit" << endl;
                hrac.hp = hrac.hp - cil->powerhit;
            } else {
                cout << "Goblin ti dal normalni utok" << endl;
                hrac.hp = hrac.hp - cil->attack;
            }

            cout << "Zbylo ti: " << hrac.hp << "hp" << endl;
            cout << "Mas " << hrac.mana << " many" << endl;
        }
    }

    if(hrac.hp < 1){
        cout << "UMREL JSI";
        return 0;
    }

    /*----11 SOUBOJ S HLAVNÍM BOSEM-----*/

cout << "Narazil jsi na hlavniho bosse - Bloomshade! Rostlinny parazit, co se maskuje jako kvetina." << endl;
cout << "Pokud ho porazis stanes se legendou, pokud prohrajes, zemres a tve jmeno bude navzdy zapomenuto." << endl;
cout << hrac.ability << " 4) Pockas nez se boss uklidni" << endl;

bool nastvany = false;
int bonus_poskozeni = 0;
int cekani = 0;
bool prvni_tah = true;

while (bbos.hp > 0 && hrac.hp > 0) {
    cout << endl << "Co mu provedeme ted: ";
    cin >> vyber_ability;

    bool utocil_tento_tah = false;
    bool uspesna_akce = false;

    if (vyber_ability == 4) {
        cout << "Cekas a vyckavas na reakci kvetiny..." << endl;
        uspesna_akce = true;
    }
    else if (vyber_postavy == 1) {
        if (vyber_ability == 1 && hrac.mana >= hrac.abilityManaCost[0]) {
            cout << "Pouzil jsi Powerhit a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
            hrac.mana -= hrac.abilityManaCost[0]; bbos.hp -= hrac.abilityDamage[0]; utocil_tento_tah = true; uspesna_akce = true;
        } else if (vyber_ability == 2 && hrac.mana >= hrac.abilityManaCost[1]) {
            cout << "Pouzil jsi Basic attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
            hrac.mana -= hrac.abilityManaCost[1]; bbos.hp -= hrac.abilityDamage[1]; utocil_tento_tah = true; uspesna_akce = true;
        } else if (vyber_ability == 3 && hrac.mana >= hrac.abilityManaCost[2]) {
            cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
            hrac.mana -= hrac.abilityManaCost[2]; bbos.hp -= hrac.abilityDamage[2]; utocil_tento_tah = true; uspesna_akce = true;
        }
    }
    else if (vyber_postavy == 2) {
        if (vyber_ability == 1 && hrac.mana >= hrac.abilityManaCost[0]) {
            cout << "Pouzil jsi Shield a obranil jsi se na jedno kolo" << endl;
            hrac.mana -= hrac.abilityManaCost[0]; uspesna_akce = true;
        } else if (vyber_ability == 2 && hrac.mana >= hrac.abilityManaCost[1]) {
            cout << "Pouzil jsi Defend attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
            hrac.mana -= hrac.abilityManaCost[1]; bbos.hp -= hrac.abilityDamage[1]; utocil_tento_tah = true; uspesna_akce = true;
        } else if (vyber_ability == 3 && hrac.mana >= hrac.abilityManaCost[2]) {
            cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
            hrac.mana -= hrac.abilityManaCost[2]; bbos.hp -= hrac.abilityDamage[2]; utocil_tento_tah = true; uspesna_akce = true;
        }
    }
    else if (vyber_postavy == 3) {
        if (vyber_ability == 1 && hrac.mana >= hrac.abilityManaCost[0]) {
            cout << "Pouzil jsi Fireball a udelil jsi " << hrac.abilityDamage[0] << " damage" << endl;
            hrac.mana -= hrac.abilityManaCost[0]; bbos.hp -= hrac.abilityDamage[0]; utocil_tento_tah = true; uspesna_akce = true;
        } else if (vyber_ability == 2 && hrac.mana >= hrac.abilityManaCost[1]) {
            cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[1] << " hp" << endl;
            hrac.mana -= hrac.abilityManaCost[1]; hrac.hp -= hrac.abilityDamage[1]; uspesna_akce = true;
        } else if (vyber_ability == 3 && hrac.mana >= hrac.abilityManaCost[2]) {
            cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
            hrac.mana -= hrac.abilityManaCost[2]; bbos.hp -= hrac.abilityDamage[2]; utocil_tento_tah = true; uspesna_akce = true;
        }
    }
    else if (vyber_postavy == 4) {
        if (vyber_ability == 1 && hrac.mana >= hrac.abilityManaCost[0]) {
            cout << "Pouzil jsi Heal a vylecil jsi se o " << -hrac.abilityDamage[0] << " hp" << endl;
            hrac.mana -= hrac.abilityManaCost[0]; hrac.hp -= hrac.abilityDamage[0]; uspesna_akce = true;
        } else if (vyber_ability == 2 && hrac.mana >= hrac.abilityManaCost[1]) {
            cout << "Pouzil jsi Attack a udelil jsi " << hrac.abilityDamage[1] << " damage" << endl;
            hrac.mana -= hrac.abilityManaCost[1]; bbos.hp -= hrac.abilityDamage[1]; utocil_tento_tah = true; uspesna_akce = true;
        } else if (vyber_ability == 3 && hrac.mana >= hrac.abilityManaCost[2]) {
            cout << "Pouzil jsi Ultimate a udelil jsi " << hrac.abilityDamage[2] << " damage" << endl;
            hrac.mana -= hrac.abilityManaCost[2]; bbos.hp -= hrac.abilityDamage[2]; utocil_tento_tah = true; uspesna_akce = true;
        }
    }

    if (!uspesna_akce) {
        cout << "Nemate dost many nebo neplatna volba, zvolte jinou abilitu." << endl;
        continue;
    }

    if (bbos.hp <= 0) break;

    if (utocil_tento_tah) {
        cekani = 0;
        if (!nastvany) {
            nastvany = true;
            cout << "!!! Bloomshade se probouzi k zivotu a stava se agresivni!!!" << endl;
        }
    } else {
        cekani++;
    }

    if (cekani >= 2 && nastvany) {
        nastvany = false;
        bonus_poskozeni = 0;
        cekani = 0;
        cout << "Bloomshade se uklidnil a jeho kvety ztraceji na agresivite." << endl;
    }

    if (prvni_tah) {
        cout << "Bloomshade te laka bliz" << endl;
        prvni_tah = false;
    }
    else if (!nastvany) {
        cout << "Bloomshade neni vyprovokovany a nedela nic" << endl;
    }
    else {
        int random_cislo_uhyb = rand() % 100 + 1;
        int aktualni_dmg = bbos.attack + bonus_poskozeni;

        if (random_cislo_uhyb <= 30) {
            cout << "Uhybl jsi! Bloomshade te svymi slehy netrefil." << endl;
        } else {
            cout << "Bloomshade utoci svymi kvetinovymi upsaly! Udeluje ti " << aktualni_dmg << " damage." << endl;
            hrac.hp -= aktualni_dmg;
        }

        bonus_poskozeni += 5;
        cout << "Bloomshade duplikuje sve kvety! Do dalsiho kola se jeho poskozeni zvysuje o 5." << endl;
    }

    cout << "---------------------------------" << endl;
    cout << "Tobe zbylo: " << hrac.hp << " hp | " << hrac.mana << " many" << endl;
    cout << "Bloomshade zbylo: " << bbos.hp << " hp" << endl;
    cout << "---------------------------------" << endl;
}

if (hrac.hp <= 0) {
    cout << "ZEMREL JSI..." << endl;
    return 0;
} else {
    cout << "Gratuluji, porazil jsi hlavniho bosse Bloomshade a stal ses legendou!" << endl;
}
}
